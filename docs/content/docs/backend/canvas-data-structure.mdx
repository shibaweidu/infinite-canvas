...
- `nodes`：画布节点列表。
- `connections`：节点连线列表。
- `groups`：画布分组列表，用来记录一组节点的组织关系和组名称。
...
type CanvasNodeData = {
  id: string;
  type: "image" | "text" | "config" | "video" | "agent";
  title: string;
...
- `id`：节点 ID。
- `type`：节点类型，当前有图片、文本、生成配置、视频、音频、智能体、项目设定、角色板和分镜板等类型。
- `title`：节点标题。
...
type CanvasNodeMetadata = {
  ...
  agentOutputFormat?: "plain" | "markdown" | "json" | "promptList";
  agentResultNodeId?: string;
  agentTasks?: { id: string; title: string; status: "idle" | "running" | "success" | "error" }[];
...
- 智能体节点：`agentName/agentInstruction/agentOutputFormat/model/prompt` 保存智能体名称、身份设定、输出格式、模型和任务；`agentTasks/agentProgress/agentCurrentStep` 只记录子任务和进度；最终内容不再写入智能体自身 `content`，而是写入 `agentResultNodeId` 指向的下游结果节点，角色 Agent 会优先写入角色板。
...
第一期分组只作为画布组织容器：`nodeIds` 记录组内节点，组框根据组内节点实时计算包围区域；拖动组标题会整体移动组内节点；解除分组只删除组记录，不删除节点和连线。
