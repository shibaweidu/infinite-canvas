...
+ [优化] 优化画布连线交互。
+ [优化] 优化模型选择用户偏好。
+ [新增] 画布视频节点支持参考图传递模式：文生视频、首帧、首尾帧、全能参考，可在节点上直接上传或从素材库选取参考图。
+ [修复] 修复视频节点重试时丢失参考图的问题。
...
+ [新增] 新增用户账号与积分体系，支持账号密码注册登录、Linux.do OAuth。
+ [新增] 管理后台公开配置支持设置模型积分、支持计费查询。
+ [新增] 画布右上角展示用户积分余额，生成按钮会展示本次预计消耗积分。
+ [新增] 新增视频生成节点。
