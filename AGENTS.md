...
- 管理后台 UI 色系统一使用黑色、灰色、白色；不要使用蓝色、青色等彩色强调色。按钮、标签、选中态必须保证文字和背景对比清晰，不能出现白字白底。
- 管理后台自定义暗色页面不要使用纯白 `bg-white` 作为主按钮、选中 Tab、选中卡片或开关轨道背景；优先使用深灰底、浅色字和灰色边框，纯白只可用于小面积图标、LOGO 或开关圆点。
- 前台和后台所有浅色按钮、选中金额块、确认按钮等浅底元素必须显式使用深色文字；深色业务面板不要使用接近纯黑的大面积背景，优先使用可分层的深灰色，避免黑色区域过重或白底白字。
- 前台任何位置都不得显示后台模型渠道的接口地址、端点、API Key、Token 等敏感信息；模型下拉、日志、诊断信息、复制内容中只显示模型名称或已脱敏信息。
- 所有可点击元素，包括链接、按钮、菜单项、头像入口、卡片入口和图标按钮，都必须在 hover 时显示手型；优先在组件上显式添加 `cursor-pointer`，并保留全局 CSS 兜底。
...
- 画布分组顶部导航和节点悬浮导航统一使用同一套胶囊工具栏样式，并保持屏幕视觉尺寸固定，不随画布缩放变大或变小。
