...
export function CanvasAssistantPanel({ nodes, selectedNodeIds, snapshot, sessions, activeSessionId, onSelectNodeIds, onSessionsChange, onApplyOps, canUndoOps, onUndoOps, onPasteImage, agentMode, onAgentModeChange, autoConnectLocal, closing, onCollapse }: CanvasAssistantPanelProps) {
    ...
    const onlineContent = (
        <>
            ...
            {view === "setup" ? (
                <OnlineAgentSetupView theme={theme} config={effectiveConfig} value={effectiveConfig.textModel} onChange={(model) => updateConfig("textModel", model)} onOpenConfig={() => openConfigDialog(true)} />
            ) : (
                <div className="thin-scrollbar min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
                    {view === "history" ? (
                        ...
                        <OnlineAgentLogView logs={onlineLogs} theme={theme} context={{ model: modelOptionName(activeModel), running: isRunning, confirmTools, messages: messages.length, nodes: snapshot.nodes.length, connections: snapshot.connections.length }} onClear={() => setOnlineLogs([])} />
                    ) : messages.length ? (
                        <>
...
function AgentTextModelPicker({ config, value, onChange }: { config: AiConfig; value: string; onChange: (model: string) => void }) {
    return <ModelPicker config={config} value={value} onChange={onChange} modelType="text" className="!h-8 !rounded-lg !px-2.5 !text-xs" placeholder="选择文本模型" />;
}
...
function OnlineAgentSetupView({ theme, config, value, onChange, onOpenConfig }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; config: AiConfig; value: string; onChange: (model: string) => void; onOpenConfig: () => void }) {
    return (
                ...
                <div className="rounded-lg border p-3" style={{ borderColor: theme.node.stroke }}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-2">
                                <ModelPicker config={config} value={value} onChange={onChange} modelType="text" fullWidth placeholder="选择文本模型" />
                            </div>
                        </div>
